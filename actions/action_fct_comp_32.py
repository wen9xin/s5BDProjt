    ...
    def refreshResult(self):
        ...
        if not self.ui.lineEdit.text().strip():
            ...
            try:
                ...
                result = cursor.execute("SELECT nomSp, prenomSp, pays, categorieSp,dateNaisSp FROM LesSportifs_base JOIN LesEquipiers USING (numSp) WHERE numEq = ?", [self.ui.lineEdit.text().strip()])
